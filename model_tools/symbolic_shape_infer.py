...
class SymbolicShapeInference:
    ...
    def infer_shapes(in_mp, int_max=2**31 - 1, auto_merge=False, guess_output_rank=False, verbose=0):
        ...
        logger.info("Model opset: " + str(onnx_opset))
...
if __name__ == "__main__":
    ...
    logger.info("[symbolic_shape_infer] Input model: " + args.input)
    if args.output:
        logger.info("[symbolic_shape_infer] Output model " + args.output)
    logger.info("[symbolic_shape_infer] Doing symbolic shape inference...")
    ...
    logger.info(f"[symbolic_shape_infer] output_mp opset: {get_opset(out_mp)}")

...
// ==================== Additional ====================
Eigen::Vector3f RotationMatrix2Vector(const Eigen::Matrix3f &rotation) {
  Eigen::AngleAxisf AA(rotation);
  Eigen::Vector3f vec = AA.angle() * AA.axis();
  return vec;
}

void RegionModality::set_kl_threshold(float kl) {
  kl_threshold_ = kl;
}
...
float RegionModality::EvaluateDistribution() {
  if(!IsSetup()) return 0.f;
  CalculateBeforeCameraUpdate();
  CalculateCorrespondences(0);

  if (data_lines_.empty()) return 0.f;

  float valid_cnt = 0.f;
  for(auto &data_line : data_lines_) {
    float kl = FunctionKL(data_line.mean, data_line.standard_deviation);
    if (kl < kl_threshold_) valid_cnt += 1.f;
  }
  return valid_cnt / data_lines_.size();
}

void RegionModality::ResetOcclusionMask(const cv::Mat &occlusion_mask) {
  occlusion_mask_ = occlusion_mask;
}


int RegionModality::len_data_lines() {
  return static_cast<int>(data_lines_.size());
}

const int RegionModality::get_n_lines() {
  return n_lines_;
}
...
void RegionModality::PrecalculateDistributionVariables() {
  ...
  min_standard_deviation_ = std::sqrt(min_variance_);
}
...
void RegionModality::PrecalculateCameraVariables() {
  ...
  image_width_minus_1_ = camera_ptr_->intrinsics().width - 1;
  image_height_minus_1_ = camera_ptr_->intrinsics().height - 1;
  image_width_minus_2_ = camera_ptr_->intrinsics().width - 2;
  image_height_minus_2_ = camera_ptr_->intrinsics().height - 2;
  ...
  if (display_visualization_) {
    ...
    if (key == 's') {
      ...
      std::filesystem::path path{
          (title + "_" + std::to_string(save_index++) + ".png")};

...
class RegionModality {
 ...
 public:
  ...
  void ResetOcclusionMask(const cv::Mat &occlusion_mask = cv::Mat());
  int len_data_lines();
  const int get_n_lines();

  cv::Mat occlusion_mask_{}; // occlusion mask
 ...
 private:
  ...
  bool visualize_lines_correspondence_ = true;

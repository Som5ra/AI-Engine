...
class KeyPoint2DResult : public PostProcessResult {
    public:
...
class RTMPose : public BaseONNX {
    public:
        RTMPose(const std::string& model_path, const std::string& config_path);
        RTMPose(std::unique_ptr<basic_model_config> _config);
        // RTMPose(std::unique_ptr<basic_model_config>& _config);
        // std::vector<float> preprocess_img(const cv::Mat& image);
        // std::vector<float> preprodcess(const cv::Mat& image);
